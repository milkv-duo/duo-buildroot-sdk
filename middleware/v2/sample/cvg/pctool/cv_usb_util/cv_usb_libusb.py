...
class cv_usb_libusb:
    ...
    def libusb_write(self, command, recv_ack, delay_ms):
        ...
        try:
            ...
            if '[Errno 32]' in str(e):
                print ("clear halt!")
                self.device.clear_halt(self.epOut)
            else:
                print (e)
    ...
    def usb_send_file(self, filename, dest_addr, delay_ms):
        ...
        while (complete_cnt < 1): # For stress test
            ...
            if content_size == 0:
                zero_p = array('B')
                send_len = self.libusb_write(zero_p, 0, delay_ms)
                return 0
    ...
    def libusb_read(self, size):
        try:
            return self.epIn.read(size, timeout=50000000)
        except usb.USBError as e:
            if '[Errno 32]' in str(e):
                print ("clear halt!")
                self.device.clear_halt(self.epIn)
            else:
                print (e)
            self.ioTime = self.ioTime + time.time() - start_time;
            return None

    def usb_read_file(self, filename, total_len):
        self.ioTime = 0
        complete_cnt = 0
        while (complete_cnt < 1): # For stress test
            complete_cnt = complete_cnt + 1
            rx_len = 1048576
            content_file = open(filename, 'wb')
            content_size = total_len
            ...
            while (content_size > 0):
                ...
                if (content_size < rx_len):
                    rx_len = content_size
                start_time = time.time()
                self.data = self.libusb_read(rx_len)
                self.ioTime = self.ioTime + time.time() - start_time;
                if self.data is not None:
                    read_len = len(self.data)
                    content_size -= read_len
                    self.data.write(content_file)
                else:
                    self.data = array('B')
                    print ("IO error, stop send file")
                    return -1
