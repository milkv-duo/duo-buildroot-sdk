...
int main(int argc, char **argv)
{
	...
	ret |= CVI_IVE_Dilate(handle, &src, &dst, &stCtrlDilate, 1);
	gettimeofday(&t1, NULL);
	elapsed_cpu =
		((t1.tv_sec - t0.tv_sec) * 1000000 + t1.tv_usec - t0.tv_usec);
	printf("%s CPU time %lu\n", __func__, elapsed_cpu);

	CVI_IVE_WriteImg(handle, "sample_Dilate_3x3_dilate_only.bin", &dst);

	memcpy(stCtrlDilate.au8Mask, arr5by5, sizeof(CVI_U8) * 25);

	printf("Run HW IVE Dilate 5x5.\n");
	ret |= CVI_IVE_Dilate(handle, &src, &dst, &stCtrlDilate, 1);
	CVI_IVE_WriteImg(handle, "sample_Dilate_5x5_dilate_only.bin", &dst);
