...
int main(int argc, char **argv)
{
	...
	CVI_IVE_CreateImage(handle, &stEdge, IVE_IMAGE_TYPE_U8C1, input_w,
			    input_h);
	memset((CVI_U8 *)(uintptr_t)stEdge.u64VirAddr[0], 0, input_w * input_h);
	...
	memset((CVI_U8 *)(uintptr_t)stStack.u64VirAddr, 0, stStack.u32Size);
	...
	// Run HW IVE.
	printf("Run HW IVE CannyHysEdge 3x3.\n");
	gettimeofday(&t0, NULL);
	...
	CVI_IVE_WriteMem(handle, "sample_Canny_Mem_3x3_0.bin", &stStack);
	CVI_IVE_WriteImg(handle, "sample_CannyHysEdge_3x3.yuv", &stEdge);

	printf("Run HW IVE CannyHysEdge 5x5.\n");
	stCannyHysEdgeCtrl.u16LowThr = 108;
	stCannyHysEdgeCtrl.u16HighThr = 3 * stCannyHysEdgeCtrl.u16LowThr;
	memcpy(stCannyHysEdgeCtrl.as8Mask, arr5by5, 5 * 5 * sizeof(CVI_S8));
	ret |= CVI_IVE_CannyHysEdge(handle, &src, &stEdge, &stStack,
				    &stCannyHysEdgeCtrl, 1);
	CVI_IVE_WriteMem(handle, "sample_Canny_Mem_5x5_0.bin", &stStack);
	CVI_IVE_WriteImg(handle, "sample_CannyHysEdge_5x5.yuv", &stEdge);
