...
int main(int argc, char **argv)
{
	...
	CVI_IVE_ReadRawImage(handle, &src1, (char *)filename, IVE_IMAGE_TYPE_U8C1,
			  input_w, input_h);
	...
	gettimeofday(&t0, NULL);
	ret = CVI_IVE_Sub(handle, &src1, &src2, &dst, &iveSubCtrl, 1);
	gettimeofday(&t1, NULL);
	elapsed_cpu =
		((t1.tv_sec - t0.tv_sec) * 1000000 + t1.tv_usec - t0.tv_usec);
	printf("%s CPU time %lu\n", __func__, elapsed_cpu);

	CVI_IVE_WriteImg(handle, "sample_Sub_Abs.yuv", &dst);
