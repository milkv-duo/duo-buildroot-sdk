...
int f37p_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunF37P_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, f37p_i2c_addr);
	if (ret < 0) {
...
int f37p_read_register(VI_PIPE ViPipe, int addr)
{
	...
	ret = write(g_fd[ViPipe], buf, f37p_addr_byte);
	if (ret < 0) {
	...
	buf[0] = 0;
	ret = read(g_fd[ViPipe], buf, f37p_data_byte);
	if (ret < 0) {
...
int f37p_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	ret = write(g_fd[ViPipe], buf, f37p_addr_byte + f37p_data_byte);
	if (ret < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C_WRITE error!\n");
		return CVI_FAILURE;
	}
