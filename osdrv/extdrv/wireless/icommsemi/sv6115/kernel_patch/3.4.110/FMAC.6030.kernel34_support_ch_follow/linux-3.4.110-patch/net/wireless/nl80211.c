			      ...
			      struct cfg80211_registered_device *dev)
{
	...
	for (band = 0; band < IEEE80211_NUM_BANDS; band++) {
		...
		/* add HE info */
		if (dev->wiphy.bands[band]->n_iftype_data) {
			...
			for (i = 0; i < dev->wiphy.bands[band]->n_iftype_data; i++) {
				...
				err = nl80211_send_iftype_data(msg,
						&dev->wiphy.bands[band]->iftype_data[i]);
				if (err)
					return err;
				  ...
				  size_t len)
{
	struct wireless_dev *wdev = dev->ieee80211_ptr;
	struct wiphy *wiphy = wdev->wiphy;
	struct cfg80211_registered_device *rdev = container_of(wiphy, struct cfg80211_registered_device, wiphy);
