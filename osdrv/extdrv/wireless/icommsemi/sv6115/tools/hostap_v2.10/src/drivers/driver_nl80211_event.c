...
*/
static int ssv_channel_change_event(struct wpa_driver_nl80211_data *drv, const struct ieee80211_mgmt *mgmt, u16 reason_code)
{
		...
		&&(memcmp(mgmt->sa, mac, 6)==0))
	{
		...
		wpa_printf(MSG_ERROR, "nl80211:ssv_channel_change_event channel %d, second_channel %d\n", channel, second_channel);
		return 1;
	}
	return 0;
}
					 ...
					 const u8 *frame, size_t len)
{
	...
	if (type == EVENT_UNPROT_DISASSOC) {
		...
		if(ssv_channel_change_event(drv,mgmt,reason_code))
			return;
		       ...
		       struct nlattr *wmm, struct nlattr *req_ie)
{
	...
	if (cmd != NL80211_CMD_FRAME_TX_STATUS && !(data[4] & 0x01) &&
		...
		ssv_channel_change_event(drv, mgmt, le_to_host16(mgmt->u.deauth.reason_code));
		}
...
int process_ssv_event(struct nl_msg *msg, void *arg)
{
	struct nl80211_global *global = arg;
	...
	union wpa_event_data event;
	struct ssv_external_auth_params *param;

	nla_parse(tb, NL80211_ATTR_MAX, genlmsg_attrdata(gnlh, 0),
		  genlmsg_attrlen(gnlh, 0), NULL);

	if (tb[E_SSV_WPAS_CTL_ATTR_EXTAUTH_RSP]) {
		param = nla_data(tb[E_SSV_WPAS_CTL_ATTR_EXTAUTH_RSP]);
		if ((param == NULL) || 
		    (nla_len(tb[E_SSV_WPAS_CTL_ATTR_EXTAUTH_RSP]) != 
		    sizeof(struct ssv_external_auth_params)))
			return NL_SKIP;

		os_memset(&event, 0, sizeof(event));
		switch (param->action) {
			case SSV_EXTERNAL_AUTH_START:
				event.external_auth.action = EXT_AUTH_START;
				break;
			case SSV_EXTERNAL_AUTH_ABORT:
				event.external_auth.action = EXT_AUTH_ABORT;
				break;
			default:
				break;
		}
		event.external_auth.key_mgmt_suite = param->key_mgmt_suite;
		event.external_auth.ssid_len = param->ssid.ssid_len;
		if (event.external_auth.ssid_len > SSID_MAX_LEN)
			return NL_SKIP;
		event.external_auth.ssid = param->ssid.ssid;
		event.external_auth.bssid = param->bssid;
		if (global != NULL) {
			drv = dl_list_first(&global->interfaces, struct wpa_driver_nl80211_data, list);
			if (drv != NULL) {
				wpa_printf(MSG_DEBUG, "%s: Send EVENT_EXTERNAL_AUTH", __func__);
				wpa_supplicant_event(drv->ctx, EVENT_EXTERNAL_AUTH, &event);
			} else {
				wpa_printf(MSG_ERROR, "%s: drv is NULL", __func__);
			}
		} else {
			wpa_printf(MSG_ERROR, "%s: global is NULL", __func__);
		}
	}
