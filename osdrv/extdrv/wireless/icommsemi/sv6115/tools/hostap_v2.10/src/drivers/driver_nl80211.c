	...
	struct i802_bss *bss)
{
	...
	if ((drv->global->ssv->ssvctrl_id > 0) && 
            (params->auth_alg & WPA_AUTH_ALG_SAE) && 
            (drv->extauth_enable != 0)) {
		wpa_printf(MSG_DEBUG, "Not support SAE, change auth_alg to open here.");
...
static int wpa_driver_nl80211_extauth_enable(void *priv)
{
	struct external_auth auth_test;
	const u8 bssid_test[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

	memset(&auth_test, 0x00, sizeof(struct external_auth));
	auth_test.action = 0;
	auth_test.status = 0xFF;
	auth_test.ssid = (u8 *)"123";
	auth_test.ssid_len = 3;
	auth_test.bssid = bssid_test;

	return nl80211_send_external_auth_status(priv, &auth_test);
}
	...
	void *priv, struct wpa_driver_associate_params *params)
{
	...
	if (!(drv->capa.flags & WPA_DRIVER_FLAGS_SME)) {
		...
		drv->extauth_enable = wpa_driver_nl80211_extauth_enable(priv);
...
static int wpa_driver_ssv_init_nl_global(struct nl80211_global *global)
{
	int ret;
	struct nl80211_global gbl_temp;

	if (global == NULL || global->ssv == NULL)
		return -1;

	global->ssv->nl_cb = nl_cb_alloc(NL_CB_DEFAULT);
	if (global->ssv->nl_cb == NULL) {
		wpa_printf(MSG_DEBUG, "ssvctrl: Failed to allocate netlink "
			   "callbacks");
		return -1;
	}

	global->ssv->nl = nl_create_handle(global->ssv->nl_cb, "nl");
	if (global->ssv->nl == NULL)
		goto err;

	global->ssv->ssvctrl_id = genl_ctrl_resolve(global->ssv->nl, SSV_CTRL_NL_NAME);
	if (global->ssv->ssvctrl_id < 0) {
		wpa_printf(MSG_DEBUG, "ssvctrl: '%s' generic netlink not "
			   "found", SSV_CTRL_NL_NAME);
		goto err;
	}

	global->ssv->nlctrl_id = genl_ctrl_resolve(global->ssv->nl, "nlctrl");
	if (global->ssv->nlctrl_id < 0) {
		wpa_printf(MSG_DEBUG,
			   "ssvctrl: 'nlctrl' generic netlink not found");
		goto err;
	}

	global->ssv->nl_event = nl_create_handle(global->ssv->nl_cb, "event");
	if (global->ssv->nl_event == NULL)
		goto err;

	gbl_temp.nlctrl_id = global->ssv->nlctrl_id;
	gbl_temp.nl_cb = global->ssv->nl_cb;
	gbl_temp.nl = global->ssv->nl;
	ret = nl_get_multicast_id(&gbl_temp, SSV_CTRL_NL_NAME, "extauth");
	if (ret >= 0)
		ret = nl_socket_add_membership(global->ssv->nl_event, ret);
	if (ret < 0) {
		wpa_printf(MSG_DEBUG, "ssv: Could not add multicast "
			   "membership for extauth events: %d (%s)",
			   ret, nl_geterror(ret));
		goto err;
	}

	nl_cb_set(global->ssv->nl_cb, NL_CB_SEQ_CHECK, NL_CB_CUSTOM,
		  no_seq_check, NULL);
	nl_cb_set(global->ssv->nl_cb, NL_CB_VALID, NL_CB_CUSTOM,
		  process_ssv_event, global);

	nl80211_register_eloop_read(&global->ssv->nl_event,
				    wpa_driver_nl80211_event_receive,
				    global->ssv->nl_cb, 0);
...
err:
	nl_destroy_handles(&global->ssv->nl_event);
	nl_destroy_handles(&global->ssv->nl);
	nl_cb_put(global->ssv->nl_cb);
	global->ssv->nl_cb = NULL;
	return -1;
}

static int ssv_global_init(struct nl80211_global *global)
{
	...
	global->ssv = os_zalloc(sizeof(struct ssvctrl));
	if (global->ssv == NULL)
		return -1;
	...
	cfg->ctx = global;
	global->ssv->netlink = netlink_init(cfg);
	if (global->ssv->netlink == NULL) {
		os_free(cfg);
		goto err;
	}
...
static void ssv_global_deinit(struct nl80211_global *global)
{
	if (global == NULL || global->ssv == NULL)
		return;

	if (global->ssv->netlink)
		netlink_deinit(global->ssv->netlink);

	nl_destroy_handles(&global->ssv->nl);

	if (global->ssv->nl_event)
		nl80211_destroy_eloop_handle(&global->ssv->nl_event, 0);
...
static void * nl80211_global_init(void *ctx)
{
	...
	if (wpa_driver_ssv_init_nl_global(global) < 0) {
		wpa_printf(MSG_DEBUG, "ssv: wpa_driver_ssv_init_nl_global failed!");
	}
...
static struct nl_msg *ssv_cmd_msg(struct wpa_driver_nl80211_data *drv,
				  int flags, uint8_t cmd)
{
	struct nl_msg *msg;
	struct nl80211_global gbl_temp;
	struct wpa_driver_nl80211_data drv_temp;

	msg = nlmsg_alloc();
	if (!msg || (drv->global->ssv->ssvctrl_id < 0))
		return NULL;

	gbl_temp.nl80211_id = drv->global->ssv->ssvctrl_id;
	drv_temp.global = &gbl_temp;
	if (!nl80211_cmd(&drv_temp, msg, flags, cmd)) {
		nlmsg_free(msg);
		return NULL;
	}
...
static int ssv_send_external_auth_status(struct wpa_driver_nl80211_data *drv,
					 struct external_auth *params)
{
	struct nl80211_global gbl_temp;
	struct nl_msg *msg = NULL;
	...
	msg = ssv_cmd_msg(drv, 0, E_SSV_WPAS_CTL_CMD_EXTAUTH);
	if (!msg || (drv->global->ssv->ssvctrl_id < 0)) {
		wpa_printf(MSG_INFO, "ssvctrl: %s error", __func__);
		goto fail;
	}

	nla_put_u16(msg, E_SSV_WPAS_CTL_ATTR_EXTAUTH, params->status);
	gbl_temp.nl_cb = drv->global->ssv->nl_cb;
	ret = send_and_recv(&gbl_temp, drv->global->ssv->nl, msg, NULL, NULL, NULL, NULL);
	msg = NULL;
	if (ret) {
		wpa_printf(MSG_DEBUG,
		   	   "ssvctrl: External Auth status update failed: ret=%d (%s)",
			   ret, strerror(-ret));
		goto fail;
	}
fail:
...
static int _nl80211_send_external_auth_status(void *priv,
					     struct external_auth *params)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	...
	if ((drv->extauth_enable != 0) && (drv->global->ssv->ssvctrl_id > 0))
		return ssv_send_external_auth_status(drv, params);
	else
		return nl80211_send_external_auth_status(priv, params);
}
...
const struct wpa_driver_ops wpa_driver_nl80211_ops = {
	...
	.send_external_auth_status = _nl80211_send_external_auth_status,
