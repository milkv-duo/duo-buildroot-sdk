...
void hostapd_change_channel(struct hostapd_data *hapd,u16 channel,u16 second_channel)
{
	...
	if((channel <= 14) && (channel >= 1)){
		...
		(channel == 100) || (channel == 104) || (channel == 108) ||(channel == 112)||(channel == 116)||
	...
	for (j = 0; j < iface->num_bss; j++) {
		...
		if (hapd->iconf->channel) {
			...
			wpa_printf(MSG_DEBUG, "hostapd_change_channel Mode: %s  Channel: %d second channel %d  "
				   "Frequency: %d MHz",
				   hostapd_hw_mode_txt(hapd->iconf->hw_mode),
				   hapd->iconf->channel, hapd->iconf->secondary_channel, iface->freq);
		//wpa_printf(MSG_ERROR, "hostapd_hw_get_freq %zd \n",j);
