...
typedef enum {
    ...
    BTC_BLE_MESH_ACT_NODE_SUSPEND,
    BTC_BLE_MESH_ACT_NODE_RESUME,
...
typedef union {
    struct ble_mesh_init_args {
        ...
        ssv_ble_mesh_comp_t *comp;
        SemaphoreHandle_t semaphore;
    } mesh_init;
    struct ble_mesh_node_prov_enable_args {
        ssv_ble_mesh_prov_bearer_t bearers;
    } node_prov_enable;
    ...
    struct ble_mesh_set_device_name_args {
        char name[SSV_BLE_MESH_DEVICE_NAME_MAX_LEN];
    } set_device_name;
    ...
    struct ble_mesh_provisioner_dev_add_args {
        ssv_ble_mesh_unprov_dev_add_t add_dev;
    ...
    struct ble_mesh_provisioner_set_prov_net_idx_args {
        ssv_ble_mesh_prov_data_info_t prov_data;
    } set_prov_data_info;
    struct ble_mesh_provisioner_set_node_name_args {
        int index;
        char name[SSV_BLE_MESH_NODE_NAME_MAX_LEN];
    } set_node_name;
...
int btc_ble_mesh_client_model_init(ssv_ble_mesh_model_t *model);

int32_t btc_ble_mesh_model_pub_period_get(ssv_ble_mesh_model_t *mod);

uint16_t btc_ble_mesh_get_primary_addr(void);

uint16_t *btc_ble_mesh_model_find_group(ssv_ble_mesh_model_t *mod, uint16_t addr);

ssv_ble_mesh_elem_t *btc_ble_mesh_elem_find(u16_t addr);

uint8_t btc_ble_mesh_elem_count(void);

ssv_ble_mesh_model_t *btc_ble_mesh_model_find_vnd(const ssv_ble_mesh_elem_t *elem,
        uint16_t company, uint16_t id);

ssv_ble_mesh_model_t *btc_ble_mesh_model_find(const ssv_ble_mesh_elem_t *elem,
        uint16_t id);

const ssv_ble_mesh_comp_t *btc_ble_mesh_comp_get(void);
