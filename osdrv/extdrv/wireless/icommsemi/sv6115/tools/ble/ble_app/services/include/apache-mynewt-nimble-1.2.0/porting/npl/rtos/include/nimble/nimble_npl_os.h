...
ble_npl_os_started(void)
{
    ...
    return OS_IsSchedulerActive();
}

static inline void *
ble_npl_get_current_task_id(void)
{
    return OS_TaskGetCurrHandle();
}
...
{
    OS_STATUS st = 0;
    //evq->q = xQueueCreate(CONFIG_BLE_MESH_EVENT_QUEUE_SIZE, sizeof(struct ble_npl_eventq *));
    st = OS_BufQCreate(&evq->q, CONFIG_BLE_MESH_EVENT_QUEUE_SIZE, sizeof(struct ble_npl_eventq *));
}
...
{//no caller
    ...
    return !OS_BufQWaitingSize(evq->q);
}
...
{
    return OS_SemGetCount(sem->handle);
    //return uxSemaphoreGetCount(sem->handle);//werner, no api
}
...
ble_npl_callout_is_active(struct ble_npl_callout *co)
{
    ...
    return OS_TimerIsActive(co->handle) == pdTRUE;
}

static inline ble_npl_time_t
ble_npl_callout_get_ticks(struct ble_npl_callout *co)
{
    return OS_TimerGetExpiryTime(co->handle);
    //return xTimerGetExpiryTime(co->handle);//werner, no api
}
...
{
    return ms * OS_TICK_RATE_HZ / 1000;
}

static inline uint32_t
ble_npl_time_ticks_to_ms32(ble_npl_time_t ticks)
{
    return ticks * 1000 / OS_TICK_RATE_HZ;
}
