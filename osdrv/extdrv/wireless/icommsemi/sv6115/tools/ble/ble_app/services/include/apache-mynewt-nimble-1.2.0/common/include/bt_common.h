...
#define BT_TASK_MAX_PRIORITIES      OS_TASK_HIGHEST//configMAX_PRIORITIES
...
// Mapping between SSV_LOG_LEVEL and BT_TRACE_LEVEL
...
#define BT_PRINT_E(tag, format, ...)   {ssv_log_write(SSV_LOG_ERROR,   tag, MESHLOG_FORMAT(E, format), ssv_log_timestamp(), tag, ##__VA_ARGS__); }
#define BT_PRINT_W(tag, format, ...)   {ssv_log_write(SSV_LOG_WARN,    tag, MESHLOG_FORMAT(W, format), ssv_log_timestamp(), tag, ##__VA_ARGS__); }
#define BT_PRINT_I(tag, format, ...)   {ssv_log_write(SSV_LOG_INFO,    tag, MESHLOG_FORMAT(I, format), ssv_log_timestamp(), tag, ##__VA_ARGS__); }
#define BT_PRINT_D(tag, format, ...)   {ssv_log_write(SSV_LOG_DEBUG,   tag, MESHLOG_FORMAT(D, format), ssv_log_timestamp(), tag, ##__VA_ARGS__); }
#define BT_PRINT_V(tag, format, ...)   {ssv_log_write(SSV_LOG_VERBOSE, tag, MESHLOG_FORMAT(V, format), ssv_log_timestamp(), tag, ##__VA_ARGS__); }
...
/* relate to SSV_BT_STATUS_xxx in ssv_bt_defs.h */
typedef enum {
    BT_STATUS_SUCCESS = 0,
    BT_STATUS_FAIL,
    BT_STATUS_NOT_READY,
