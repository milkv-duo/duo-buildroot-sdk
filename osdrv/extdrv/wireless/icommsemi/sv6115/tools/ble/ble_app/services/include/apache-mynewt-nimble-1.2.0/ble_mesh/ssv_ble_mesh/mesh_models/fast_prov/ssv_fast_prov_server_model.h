...
#ifndef _SSV_FAST_PROV_SERVER_MODEL_H
#define _SSV_FAST_PROV_SERVER_MODEL_H
...
typedef struct {
    ssv_ble_mesh_model_t *model;    /* Fast Prov Server model pointer */
    ...
    ssv_ble_mesh_msg_ctx_t ctx; /* the context stored for sending fast prov status message */
...
ssv_err_t example_store_remote_node_address(uint16_t node_addr);

ssv_err_t example_fast_prov_server_recv_msg(ssv_ble_mesh_model_t *model,
        ssv_ble_mesh_msg_ctx_t *ctx, struct net_buf_simple *buf);

ssv_err_t example_handle_fast_prov_info_set_comp_evt(ssv_ble_mesh_model_t *model, uint8_t status_unicast,
        uint8_t status_net_idx, uint8_t status_match);

ssv_err_t example_handle_fast_prov_action_set_comp_evt(ssv_ble_mesh_model_t *model, uint8_t status_action);

ssv_err_t example_handle_fast_prov_status_send_comp_evt(int err_code, uint32_t opcode,
        ssv_ble_mesh_model_t *model, ssv_ble_mesh_msg_ctx_t *ctx);

ssv_err_t example_fast_prov_server_init(ssv_ble_mesh_model_t *model);

#endif /* _SSV_FAST_PROV_SERVER_MODEL_H */