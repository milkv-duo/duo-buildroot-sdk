...
({                                                      \
    void *p;                                            \
    p = OS_MemCalloc(1, (size));                              \
    if (p == NULL)                                      \
        ASSERT(0);                                      \
    (void *)p;                                          \
})

#define BM_EnterCritical()      OS_EnterCritical()
#define BM_ExitCritical()      OS_ExitCritical()

#endif


#endif /* #if HEAP_ALLOCATION_FROM_SPIRAM_FIRST */
#define osi_free(p)                       OS_MemFree((p))
