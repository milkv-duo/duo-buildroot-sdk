	...
	IN PMLME_QUEUE_ELEM Elem)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		("AUTH_RSP - Rx Deauth frame from BSSID[%02x:%02x:%02x:%02x:%02x:%02x] DA[%02x:%02x:%02x:%02x:%02x:%02x] ReasonCode[0x%x]\n",
	...
	if (PeerDeauthSanity(pAd, Elem->Msg, Elem->MsgLen, Addr1, Addr2, Addr3, &Reason)) {
		if (INFRA_ON(pAd)
			...
			cfg80211_rx_mlme_mgmt(pAd->net_dev, (PUINT8)prDeauthFrame, (size_t)Elem->MsgLen);
