	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	else if (MlmeAssocReqSanity(pAd, Elem->Msg, Elem->MsgLen, ApAddr, &CapabilityInfo, &Timeout, &ListenIntv))
	{
...
#ifdef WPA3_SUPPORT
		BssIdx = BssSsidTableSearch(&pAd->ScanTab, pAd->MlmeAux.Bssid, pAd->MlmeAux.Ssid, pAd->MlmeAux.SsidLen, pAd->MlmeAux.Channel);
		if (BssIdx == BSS_NOT_FOUND) {
			DBGPRINT(RT_DEBUG_TRACE, ("ASSOC - Cannot match BssEntry in last Scan Result!\n"));
			pAd->Mlme.AssocMachine.CurrState = ASSOC_IDLE;
			Status = MLME_MAC_TABLE_FAIL;
		...
		if (!RTMPPerformPolicySelection(pAd, BssIdx, BSS0))
			DBGPRINT(RT_DEBUG_WARN, ("Bss fail for RSN check\n"));
#endif
		...
		DBGPRINT(RT_DEBUG_OFF, ("ASSOC - Send ASSOC request...\n"));
		...
		if (((wdev->AuthMode == Ndis802_11AuthModeWPAPSK) ||
...
#endif /* WPA_SUPPLICANT_SUPPORT */
			{
				...
				if (wdev->AuthMode == Ndis802_11AuthModeWPA2) {
					...
					if (FoundPMK) {
						/* Set PMK number */
						if((pAd->StaCfg.RSNIE_Len + 2 + 16) <= MAX_LEN_OF_RSNIE)
						{
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	else if (MlmeAssocReqSanity(pAd, Elem->Msg, Elem->MsgLen, ApAddr, &CapabilityInfo, &Timeout, &ListenIntv))
	{
		...
		BssIdx = BssSsidTableSearch(&pAd->ScanTab, pAd->MlmeAux.Bssid, pAd->MlmeAux.Ssid, pAd->MlmeAux.SsidLen, pAd->MlmeAux.Channel);
		if (BssIdx == BSS_NOT_FOUND) {
			DBGPRINT(RT_DEBUG_TRACE, ("REASSOC - Cannot match BssEntry in last Scan Result!\n"));
			pAd->Mlme.AssocMachine.CurrState = ASSOC_IDLE;
			Status = MLME_MAC_TABLE_FAIL;
			MlmeEnqueue(pAd, MLME_CNTL_STATE_MACHINE, MT2_REASSOC_CONF, 2, &Status, 0);
			if (pOutBuffer)
		...
		if (!RTMPPerformPolicySelection(pAd, BssIdx, BSS0))
			DBGPRINT(RT_DEBUG_WARN, ("REASSOC - Bss fail for RSN check\n"));
#endif
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if(Elem == NULL)
	{
		DBGPRINT(RT_DEBUG_OFF, ("%s():Elem == NULL!\n", __FUNCTION__));
		return;
	}
			       ...
			       &CkipFlag, ie_list))
	{
		...
		if (MAC_ADDR_EQUAL(Addr2, pAd->MlmeAux.Bssid)) {
			 ...
			 */
			DBGPRINT(RT_DEBUG_TRACE, ("Report RX Assoc to upper layer, %s\n", pAd->StaCfg.ConnInfo.bss ? "DO IT" : "Oops"));

			if (pAd->StaCfg.ConnInfo.bss) {
				...
				cfg80211_rx_assoc_resp(pAd->net_dev, pAd->StaCfg.ConnInfo.bss,
...
VOID PeerDisassocAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (PeerDisassocSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, &Reason)) {
		...
		if (INFRA_ON(pAd)
			...
			cfg80211_rx_mlme_mgmt(pAd->net_dev, (PUINT8)prDisassocFrame, (size_t)Elem->MsgLen);
