...
//#endif /* RTMP_EFUSE_SUPPORT */
	...
	/*p2p_4.2.2, avoid test bed sta1 diconnecting in 1min*/
	{"GcIdleTimeout", Set_P2pGcIdleTime_Proc},
	{NULL,}
};
...
{
	NDIS_802_11_SSID Ssid, *pSsid = NULL;
	NDIS_802_11_MAC_ADDRESS Bssid, *pBssid = NULL;	/* BSSID */
	int success = TRUE;
	ULONG BssIdx = BSS_NOT_FOUND;

	DBGPRINT(RT_DEBUG_TRACE, ("802 > %s ==>\n", __func__));
	...
	*/
	...
	CFG80211DBG(RT_DEBUG_TRACE, ("Bssid is [%02x:%02x:%02x:%02x:%02x:%02x]\n", PRINT_MAC(arg)));

	NdisZeroMemory(&Bssid, MAC_ADDR_LEN);
	NdisMoveMemory(Bssid, arg, MAC_ADDR_LEN);
	...
	BssIdx = BssTableSearch(&pAd->ScanTab, (UCHAR *)pBssid, ChanNum);
	if (BssIdx == BSS_NOT_FOUND) {
		DBGPRINT(RT_DEBUG_TRACE, ("!!!BSS_NOT_FOUND in last Scan Result!!!\n"));
		return FALSE;
	} else {
		...
		if (pAd->ScanTab.BssEntry[BssIdx].SsidLen != 0) {
			NdisMoveMemory(Ssid.Ssid, pAd->ScanTab.BssEntry[BssIdx].Ssid, pAd->ScanTab.BssEntry[BssIdx].SsidLen);
			Ssid.SsidLength = pAd->ScanTab.BssEntry[BssIdx].SsidLen;
	...
	/* Why update PMK here, not after ASSOC ok? */
	...
	pAd->StaCfg.ConnectinfoChannel = pAd->MlmeAux.Channel;
	pAd->StaCfg.Connectinfoflag = TRUE;
	NdisMoveMemory(pAd->StaCfg.ConnectinfoBssid, Bssid, MAC_ADDR_LEN);
	pAd->StaCfg.ConnectinfoSsidLen = Ssid.SsidLength;
	NdisMoveMemory(pAd->StaCfg.ConnectinfoSsid, Ssid.Ssid, Ssid.SsidLength);
...
INT Set_EncrypType_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	else if (rtstrcasecmp(arg, "SMS4") == TRUE)
    {
		...
		pAd->StaCfg.PairCipher = Ndis802_11Encryption5Enabled;
...
INT	Set_P2pGcIdleTime_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	LONG idle_time;
	...
	if (idle_time < MAC_TABLE_MIN_AGEOUT_TIME)
		...
		pAd->ApCfg.P2pGcIdleTimeout = idle_time;

	DBGPRINT(RT_DEBUG_TRACE, ("%s : IdleTimeout=%d\n", __FUNCTION__, pAd->ApCfg.P2pGcIdleTimeout));
...
INT Set_usb2_Disconnect(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	ULONG Value = simple_strtol(arg, 0, 10);
	...
	DBGPRINT(RT_DEBUG_OFF, ("Usb2 Disconnect Mode = %d\n", pAd->dp_ctrl.nUsb2DisconMode));
...
#define RTMP_STA_STANDARD_IOCTL_HANDLE(__pAd, __pData, __Data, __SubCmd)	\
	...
	case CMD_RTPRIV_IOCTL_STA_SIOCSIWGENAUTHIE:								\
		return RtmpIoctl_rt_ioctl_siwgenauthie(__pAd, __pData, __Data);		\
...
#ifdef WPA3_SUPPORT
				else if (wdev->AuthMode == Ndis802_11AuthModeWPA3SAE) {
					wdev->AuthMode = Ndis802_11AuthModeWPA2;
#ifdef WPA_SUPPLICANT_SUPPORT
					wdev->IEEE8021X = FALSE;
#endif /* WPA_SUPPLICANT_SUPPORT */
    ...
    IN  UCHAR			*pKey)
{
	...
	if (!memcmp(pAd->SharedKey[BSS0][keyIdx].Key, pKey, LEN_TK)) {
		DBGPRINT(RT_DEBUG_OFF, ("%s skip reinstall %cTK\n", __func__,
			(bGTK) ? 'G' : 'P'));
	...
	if (!bGTK)
	{
		...
		/* clear rx PN for comparing later */
		...
		RTMPSetWcidSecurityInfo(pAd,
	    						BSS0,
									...
									(UCHAR)pEntry->wcid,
									PAIRWISEKEYTABLE);
		/* Frag Attack: Clear Frag Buffer to avoid AMSDU Attack */
		DBGPRINT(RT_DEBUG_ERROR, ("%s: Clear FragBuffer!\n", __func__));
		RESET_FRAGFRAME(pAd->FragFrame);
		DBGPRINT(RT_DEBUG_OFF, ("install PTK %d\n", keyIdx));
#ifdef MT_MAC
        if (pAd->chipCap.hif_type == HIF_MT)
	...
	{
		DBGPRINT(RT_DEBUG_OFF, ("install GTK %d\n", keyIdx));
...
========================================================================
Routine Description:
	Handler for CMD_RTPRIV_IOCTL_STA_SIOCSIWGENAUTHIE.
...
*/
INT
RtmpIoctl_rt_ioctl_siwgenauthie(
	IN	RTMP_ADAPTER			* pAd,
	IN	VOID					*pData,
	IN	ULONG					Data)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("===> rt_ioctl_siwgenauthie, AuthIeLen %ld\n", Data));
	if ((length > 0) && (pData == NULL)) {
		return NDIS_STATUS_FAILURE;
	} else if (length) {
		if (pAd->StaCfg.wpa_supplicant_info.pWpaAuthIe) {
			os_free_mem(NULL, pAd->StaCfg.wpa_supplicant_info.pWpaAuthIe);
			pAd->StaCfg.wpa_supplicant_info.pWpaAuthIe = NULL;
		}
/*		pAd->StaCfg.pWpaAssocIe = kmalloc(wrqu->data.length, MEM_ALLOC_FLAG); */
		os_alloc_mem(NULL, (UCHAR **)&pAd->StaCfg.wpa_supplicant_info.pWpaAuthIe, length);
		if (pAd->StaCfg.wpa_supplicant_info.pWpaAuthIe) {
			pAd->StaCfg.wpa_supplicant_info.WpaAuthIeLen = length;
			NdisMoveMemory(pAd->StaCfg.wpa_supplicant_info.pWpaAuthIe, pData, pAd->StaCfg.wpa_supplicant_info.WpaAuthIeLen);
		} else
			pAd->StaCfg.wpa_supplicant_info.WpaAuthIeLen = 0;
	}
	return NDIS_STATUS_SUCCESS;
}
#endif
