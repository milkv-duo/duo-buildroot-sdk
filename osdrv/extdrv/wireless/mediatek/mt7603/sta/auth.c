	...
	IN MLME_QUEUE_ELEM * pElem)
{
	...
	if (!AUTH_Send(pAd, pElem, &pAd->MlmeAux.AuthTimer, "AUTH")) {
	...
	IN PCHAR pSMName)
{
	...
	if (pAd->StaCfg.bBlockAssoc == TRUE) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s - Block Auth Commit durning WPA block period!\n",
			  pSMName));
		pAd->Mlme.AuthMachine.CurrState = AUTH_REQ_IDLE;
		Status = MLME_STATE_MACHINE_REJECT;
		MlmeEnqueue(pAd, MLME_CNTL_STATE_MACHINE, MT2_AUTH_CONF, 2,
		...
		if (NStatus != NDIS_STATUS_SUCCESS) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s - MlmeAuthAction(Alg:%d) allocate memory failed\n",
				  pSMName, Alg));
			pAd->Mlme.AuthMachine.CurrState = AUTH_REQ_IDLE;
			Status = MLME_FAIL_NO_RESOURCE;
			MlmeEnqueue(pAd, MLME_CNTL_STATE_MACHINE, MT2_AUTH_CONF,
		...
		MgtMacHeaderInit(pAd, &AuthHdr, SUBTYPE_AUTH, 0, Addr,
							pAd->CurrentAddress,
							pAd->MlmeAux.Bssid);
		if (pAd->StaCfg.ConnInfo.ucAuthDataLen != 0) {
			DBGPRINT(RT_DEBUG_TRACE, ("%s - Send AUTH (Seq=%d Alg=%d)...\n", pSMName, pAd->StaCfg.ConnInfo.aucAuthData[0], Alg));
			MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11),
					&AuthHdr, 2, &Alg, pAd->StaCfg.ConnInfo.ucAuthDataLen, &pAd->StaCfg.ConnInfo.aucAuthData,
					END_OF_ARGS);
			if (Alg == AUTH_MODE_SAE && pAd->StaCfg.ConnInfo.aucAuthData[0] == 1)
				pAd->MlmeAux.fgOwnAuthCommitSend = TRUE;
			else if (Alg == AUTH_MODE_SAE && pAd->StaCfg.ConnInfo.aucAuthData[0] == 2)
				pAd->MlmeAux.fgOwnAuthConfirmSend = TRUE;
		} else {
		...
		if (pAd->StaCfg.wpa_supplicant_info.WpaAuthIeLen > 0) {
			MakeOutgoingFrame(pOutBuffer + FrameLen, &tmp,
					pAd->StaCfg.wpa_supplicant_info.WpaAuthIeLen, pAd->StaCfg.wpa_supplicant_info.pWpaAuthIe, END_OF_ARGS);
			FrameLen += tmp;
		}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (PeerAuthSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, &Alg, &Seq, &Status, (PCHAR) ChlgText)) {
		DBGPRINT(RT_DEBUG_TRACE, ("Report RX AUTH to upper layer with alg:%d, SN:%d, status:%d\n", Alg, Seq, Status));
		if (MAC_ADDR_EQUAL(pAd->MlmeAux.Bssid, Addr2)) {

			RTMPCancelTimer(&pAd->MlmeAux.AuthTimer, &TimerCancelled);

			if (Alg == AUTH_MODE_SAE && Seq == 1 && Status == MLME_SUCCESS)
				pAd->MlmeAux.fgPeerAuthCommitReceived = TRUE;
			else if (Alg == AUTH_MODE_SAE && Seq == 2 && Status == MLME_SUCCESS)
				pAd->MlmeAux.fgPeerAuthConfirmReceived = TRUE;

			if (Alg == AUTH_MODE_KEY && Seq != 4 && Status == MLME_SUCCESS) {
				DBGPRINT(RT_DEBUG_TRACE,
					("WEP - Not Report Auth pkt to supplicant with Seq != 4.\n"));
				/* 2. shared key, need to be challenged */
					Seq++;
					RemoteStatus = MLME_SUCCESS;
					...
					/* TSC increment */
					INC_TX_TSC(pAd->SharedKey[BSS0][pAd->StaCfg.wdev.DefaultKeyId].TxTsc, LEN_WEP_TSC);

					/* Construct the 4-bytes WEP IV header */
					RTMPConstructWEPIVHdr(pAd->StaCfg.wdev.DefaultKeyId,
							      pAd->SharedKey[BSS0][pAd->StaCfg.wdev.DefaultKeyId].TxTsc, iv_hdr);
					...
					if (RTMPSoftEncryptWEP(pAd,
							       iv_hdr,
							       &pAd->SharedKey[BSS0][pAd->StaCfg.wdev.DefaultKeyId],
							       CyperChlgText, c_len) == FALSE) {
						MlmeFreeMemory(pAd, pOutBuffer);
						pAd->Mlme.AuthMachine.CurrState = AUTH_REQ_IDLE;
						Status2 = MLME_FAIL_NO_RESOURCE;
				...
				cfg80211_rx_mlme_mgmt(pAd->net_dev, (const u8 *)pAuthFrame, (size_t)(Elem->MsgLen));
				DBGPRINT(RT_DEBUG_TRACE, ("notification of RX Authentication Done\n"));
			}

			if (Status == MLME_SUCCESS) {
				if ((pAd->MlmeAux.Alg == Ndis802_11AuthModeOpen) ||
					(pAd->MlmeAux.Alg == Ndis802_11AuthModeShared && Seq == 4)
					|| (Alg == AUTH_MODE_SAE && pAd->MlmeAux.fgPeerAuthCommitReceived == TRUE && pAd->MlmeAux.fgPeerAuthConfirmReceived == TRUE)
					) {
					pAd->Mlme.AuthMachine.CurrState = AUTH_REQ_IDLE;
				...
				pAd->StaCfg.AuthFailReason = Status;
				COPY_MAC_ADDR(pAd->StaCfg.AuthFailSta, Addr2);
				DBGPRINT(RT_DEBUG_TRACE, ("Auth Req was rejected by [%02x:%02x:%02x:%02x:%02x:%02x], Status = %d\n", PRINT_MAC(Addr2), Status));
...
#else
	PeerAuthRspAtSeq2Action(pAd, Elem);
#endif
	LabelOK:
		if (ChlgText != NULL)
			os_free_mem(NULL, ChlgText);
...
BOOLEAN SendDeauthFrame(IN PRTMP_ADAPTER pAd)
{
	HEADER_802_11 DeauthHdr;
	PUCHAR pOutBuffer = NULL;
	NDIS_STATUS NStatus;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s - Send DEAUTH request (Reason=%d)...\n", __func__, Reason));
	MgtMacHeaderInit(pAd, &DeauthHdr, SUBTYPE_DEAUTH, 0, pAd->MlmeAux.Bssid,
			pAd->CurrentAddress,
			pAd->MlmeAux.Bssid);
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof (HEADER_802_11),
			&DeauthHdr, 2, &Reason, END_OF_ARGS);

	DBGPRINT(RT_DEBUG_TRACE, ("notification of Deauthentication %ld\n", FrameLen));
	cfg80211_rx_mlme_mgmt(pAd->net_dev, (PUINT8)pOutBuffer, (size_t)FrameLen);
	DBGPRINT(RT_DEBUG_TRACE, ("notification of Deauthentication Done\n"));

	MiniportMMRequest(pAd, 0, pOutBuffer, FrameLen);
	MlmeFreeMemory(pAd, pOutBuffer);

	pAd->StaCfg.DeauthReason = Reason;
	COPY_MAC_ADDR(pAd->StaCfg.DeauthSta, pAd->MlmeAux.Bssid);
	...
	OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(Sm, AUTH_REQ_IDLE, MT2_MLME_AUTH_REQ,
			      (STATE_MACHINE_FUNC) MlmeAuthAction);
	StateMachineSetAction(Sm, AUTH_REQ_IDLE, MT2_PEER_AUTH_EVEN,
			      (STATE_MACHINE_FUNC) PeerAuthAction);
	StateMachineSetAction(Sm, AUTH_REQ_IDLE, MT2_AUTH_TIMEOUT,
