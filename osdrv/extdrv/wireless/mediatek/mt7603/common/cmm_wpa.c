...
UCHAR       OUI_WPA2_NONE[4] = {0x00, 0x0F, 0xAC, 0x00};
...
/*WPA3 OUI*/
#ifdef WPA3_SUPPORT
UCHAR		OUI_WPA3_SAE_AKM[4]				= {0x00, 0x0F, 0xAC, 0x08};
UCHAR		OUI_WPA3_1X_SUITEB_AKM[4]				= {0x00, 0x0F, 0xAC, 0x0B};
UCHAR		OUI_WPA3_1X_SUITEB192_AKM[4]				= {0x00, 0x0F, 0xAC, 0x0C};
UCHAR		OUI_WPA3_OWE_AKM[4]				= {0x00, 0x0F, 0xAC, 0x12};
#endif
...
#ifdef WPA3_SUPPORT
UCHAR OUI_MSA_SAE_AKM[4] = {0x00, 0x0F, 0xAC, 0x08};	/* Not yet final - IEEE 802.11s-D1.06*/
#endif
#endif /* MESH_SUPPORT */
...
#ifdef WPA3_SUPPORT
UCHAR		OUI_FT_SAE_AKM[4] 		= {0x00, 0x0F, 0xAC, 0x08};
#endif
	...
	*rsn_len = 0;
	...
	if (ElementID == Wpa2Ie)
	{
		...
		if ((pAd->OpMode == OPMODE_STA) &&
			...
			&& (pAd->StaCfg.GroupCipher != Ndis802_11GCMP256Enable)
#endif
#ifdef P2P_SUPPORT
			&& (apidx == BSS0)
#endif /* P2P_SUPPORT */
	...
	OUT	UCHAR			*rsn_len)
{
	...
	if (ElementID == Wpa2Ie)
	{
...
#ifdef WPA3_SUPPORT
						|| (AuthMode == Ndis802_11AuthModeWPA3SAE)
#endif
					) {
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			...
			if (pAd->MlmeAux.MdIeInfo.Len > 0)
			{
				...
				if (AuthMode == Ndis802_11AuthModeWPA2)
					...
					NdisMoveMemory(&FtAkmOui[0], OUI_FT_SAE_AKM, 4);
#endif
				else
					AkmCnt = 0;
			}
		...
		if ((apidx >= MIN_NET_DEVICE_FOR_MESH)
			...
			if ((AuthMode == Ndis802_11AuthModeWPA2) ||(AuthMode == Ndis802_11AuthModeWPA1WPA2))
				NdisMoveMemory(pRsnie_auth->auth[0].oui, OUI_MSA_8021X_AKM, 4);
			else if((AuthMode == Ndis802_11AuthModeWPA2PSK) ||(AuthMode == Ndis802_11AuthModeWPA1PSKWPA2PSK)
#ifdef WPA3_SUPPORT
				|| (AuthMode == Ndis802_11AuthModeWPA3SAE) || (AuthMode == Ndis802_11AuthModeWPA2PSKWPA3SAE)
#endif
				)
		...
		switch (AKMSuite) {
			...
			case RSN_AKM_SUITE_SAE:
				NdisMoveMemory(pRsnie_auth->auth[0].oui, OUI_WPA3_SAE_AKM, 4);
				break;
#endif
#ifdef CFG_SUPPORT_SUITEB
			case RSN_AKM_SUITE_8021X_SUITE_B:
				NdisMoveMemory(pRsnie_auth->auth[0].oui, OUI_WPA3_1X_SUITEB_AKM, 4);
				break;
			case RSN_AKM_SUITE_8021X_SUITE_B_192:
				NdisMoveMemory(pRsnie_auth->auth[0].oui, OUI_WPA3_1X_SUITEB192_AKM, 4);
				break;
#endif
#ifdef CFG_SUPPORT_OWE
			case RSN_AKM_SUITE_OWE:
				NdisMoveMemory(pRsnie_auth->auth[0].oui, OUI_WPA3_OWE_AKM, 4);
				break;
#endif
			case RSN_AKM_SUITE_NONE:
				NdisMoveMemory(pRsnie_auth->auth[0].oui, OUI_WPA2_NONE, 4);
				break;
...
#else
	pRSN_Cap = (PUCHAR)(pRsnIe + (*rsn_len));
	pTemp_Cap = &pRsnInfo->u2RsnCap;
#endif
...
#ifdef WPA3_SUPPORT
#ifdef CFG_SUPPORT_802_11W
	NdisMoveMemory(pRSN_Cap, (PUCHAR)pTemp_Cap, sizeof(UINT16));
	DBGPRINT(RT_DEBUG_TRACE, ("Gen RSN IE Capability = %s\n", pRSN_Cap));
	if (ucRSNMfpCap == RSN_AUTH_MFP_REQUIRED) {
		pRSN_Cap[0] = (UINT8)(ELEM_WPA_CAP_MFPC | ELEM_WPA_CAP_MFPR);
		pRSN_Cap[1] = (UINT8)((ELEM_WPA_CAP_MFPC | ELEM_WPA_CAP_MFPR) >> 8);
		DBGPRINT(RT_DEBUG_TRACE, ("RSN_AUTH_MFP - MFPC & MFPR\n"));
	} else if (ucRSNMfpCap == RSN_AUTH_MFP_OPTIONAL) {
		pRSN_Cap[0] = (UINT8)(ELEM_WPA_CAP_MFPC);
		pRSN_Cap[1] = (UINT8)(ELEM_WPA_CAP_MFPC >> 8);
		DBGPRINT(RT_DEBUG_TRACE, ("RSN_AUTH_MFP - MFPC\n"));
	} else
		DBGPRINT(RT_DEBUG_TRACE, ("!RSN_AUTH_MFP - No MFPC!\n"));
#else
	/* Capabilities */
	NdisMoveMemory(pRSN_Cap, (PUCHAR)pTemp_Cap, sizeof(UINT16));
#endif
	pRSN_Cap += sizeof(UINT16);
	...
	OUT	UCHAR			*rsn_len)
{
...
#ifdef WPA3_SUPPORT
	pBuf = (pRsnIe + (*rsn_len));
	pTemp_PmkidCnt = &pRsnInfo->u2PmkidCnt;
	if (AKMSuite == RSN_AKM_SUITE_SAE || AKMSuite == RSN_AKM_SUITE_OWE) {
		...
		NdisMoveMemory(pBuf, (PUCHAR)pTemp_PmkidCnt, sizeof(UINT16));
		pBuf += 2;
		(*rsn_len) += sizeof(UINT16);

		/* Fill PMKID List field */
		if (pRsnInfo->u2PmkidCnt > 0)
			NdisMoveMemory(pBuf, pRsnInfo->aucPmkidList, (pRsnInfo->u2PmkidCnt * RSN_PMKID_LEN));

		pBuf += (pRsnInfo->u2PmkidCnt * RSN_PMKID_LEN);
		(*rsn_len) += (pRsnInfo->u2PmkidCnt * RSN_PMKID_LEN);
	}
#else
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
...
VOID RTMPMakeRSNIE(RTMP_ADAPTER *pAd, UINT AuthMode, UINT WepStatus, UCHAR apidx)
{
...
#ifdef WPA3_SUPPORT
				&& (AuthMode != Ndis802_11AuthModeWPA3SAE)
				&& (AuthMode != Ndis802_11AuthModeWPA2PSKWPA3SAE)
#endif
#ifdef WAPI_SUPPORT
...
#ifdef WPA3_SUPPORT
			AKMSuite = pAd->ApCfg.MBSSID[MAIN_MBSSID].AKMSuite;
#ifdef CFG_SUPPORT_802_11W
			ucRSNMfpCap = pAd->ApCfg.MBSSID[MAIN_MBSSID].ucRSNMfpCap;
#endif
			pRsnInfo = &pAd->ApCfg.MBSSID[MAIN_MBSSID].RsnInfo;
#endif
...
#ifdef WPA3_SUPPORT
				AKMSuite = pAd->ApCfg.ApCliTab[apcliIfidx].AKMSuite;
#ifdef CFG_SUPPORT_802_11W
				ucRSNMfpCap = pAd->ApCfg.ApCliTab[apcliIfidx].ucRSNMfpCap;
#endif
				pRsnInfo = &pAd->ApCfg.ApCliTab[apcliIfidx].RsnInfo;
#endif
				break;
			}
			else
...
#ifdef WPA3_SUPPORT
			AKMSuite = pAd->ApCfg.MBSSID[apidx].AKMSuite;
#ifdef CFG_SUPPORT_802_11W
			ucRSNMfpCap = pAd->ApCfg.MBSSID[apidx].ucRSNMfpCap;
#endif
			pRsnInfo = &pAd->ApCfg.MBSSID[apidx].RsnInfo;
#endif
...
#ifdef WPA3_SUPPORT
			AKMSuite = pAd->StaCfg.AKMSuite;
#ifdef CFG_SUPPORT_802_11W
			ucRSNMfpCap = pAd->StaCfg.ucRSNMfpCap;
#endif
...
#endif /* WAPI_SUPPORT */
	{
	        if(pRsnIe)
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s: make RSN IE Cipher!\n", __func__));
        		RTMPMakeRsnIeCipher(pAd, PrimaryRsnie, WepStatus, apidx, bMixCipher, FlexibleCipher, pRsnIe, &p_offset);

        		/* 2. insert AKM*/
				DBGPRINT(RT_DEBUG_TRACE, ("%s: make RSN IE AKM!, p_offset %d\n", __func__, p_offset));
#ifdef WPA3_SUPPORT
				RTMPMakeRsnIeAKM(pAd, PrimaryRsnie, AuthMode, AKMSuite, apidx, pRsnIe, &p_offset);
#else
				RTMPMakeRsnIeAKM(pAd, PrimaryRsnie, AuthMode, apidx, pRsnIe, &p_offset);
#endif
        		/* 3. insert capability*/
				DBGPRINT(RT_DEBUG_TRACE, ("%s: make RSN IE Capability!, p_offset %d\n", __func__, p_offset));
#ifdef WPA3_SUPPORT
				RTMPMakeRsnIeCap(pAd, PrimaryRsnie, pRsnInfo, AKMSuite, ucRSNMfpCap, apidx, pRsnIe, &p_offset);
#else
				RTMPMakeRsnIeCap(pAd, PrimaryRsnie, apidx, pRsnIe, &p_offset);
#endif
				DBGPRINT(RT_DEBUG_TRACE, ("%s: RSN IE Len %d\n", __func__, p_offset));

#ifdef DOT11W_PMF_SUPPORT
				/* 4. Insert PMKID */
#ifdef WPA3_SUPPORT
				RTMPInsertRsnIeZeroPMKID(pAd, PrimaryRsnie, AKMSuite, pRsnInfo, apidx, pRsnIe, &p_offset);
#else
...
#ifdef WPA3_SUPPORT
				PMF_MakeRsnIeGMgmtCipher(pAd, PrimaryRsnie, AKMSuite, pRsnInfo, apidx, pRsnIe, &p_offset);
#else
	...
	*rsnielen_cur_p = p_offset;
		...
		)
	{
		...
		if (bMixRsnIe)
		{
			...
			/* 2. insert AKM*/
#ifdef WPA3_SUPPORT
			RTMPMakeRsnIeAKM(pAd, Wpa2Ie, AuthMode, AKMSuite, apidx, pRsnIe_ex, &s_offset);
#else
			...
			/* 3. insert capability*/
#ifdef WPA3_SUPPORT
			RTMPMakeRsnIeCap(pAd, Wpa2Ie, pRsnInfo, AKMSuite, ucRSNMfpCap, apidx, pRsnIe_ex, &s_offset);
#else
			...
			/* 4. Insert PMKID */
#ifdef WPA3_SUPPORT
			RTMPInsertRsnIeZeroPMKID(pAd, Wpa2Ie, AKMSuite, pRsnInfo, apidx, pRsnIe_ex, &s_offset);
#else
			...
			/* 5. Insert Group Management Cipher*/
#ifdef WPA3_SUPPORT
			PMF_MakeRsnIeGMgmtCipher(pAd, Wpa2Ie, AKMSuite, pRsnInfo, apidx, pRsnIe_ex, &s_offset);
#else
...
========================================================================
	...
	Routine Description:
		Parse RSN IE from upper layer.

	Arguments:
		pAd				pointer to our pAdapter context
	prInfoElem		Pointer to the RSN IE
	prRsnInfo		Pointer to the BSSDescription structure to store the RSN information from the given RSN IE
...
========================================================================
*/
BOOLEAN RTMPParseRSNIE(IN PRTMP_ADAPTER pAd, IN P_RSN_INFO_ELEM_T prInfoElem, OUT P_RSN_INFO_T prRsnInfo)
{
	UINT32 i;
	INT32 u4RemainRsnIeLen;
	UINT16 u2Version;
	UINT16 u2Cap = 0;
	UINT32 u4GroupSuite = RSN_CIPHER_SUITE_CCMP;
	UINT32 u4GroupMgmtSuite = 0;
	UINT16 u2PairSuiteCount = 0;
	UINT16 u2AuthSuiteCount = 0;
	PUINT8 pucPairSuite = NULL;
	PUINT8 pucAuthSuite = NULL;
	PUINT8 cp;
#ifdef WPA3_SUPPORT
	UINT16 u2DesiredPmkidCnt = 0;
	UINT16 u2SupportedPmkidCnt = 0;
#endif

	/* Verify the length of the RSN IE. */
	if (prInfoElem->ucLength < 2) {
		DBGPRINT(RT_DEBUG_TRACE, ("RSN IE length too short (length=%d)\n", prInfoElem->ucLength));
		return FALSE;
	}

	/* Check RSN version: currently, we only support version 1. */
	WLAN_GET_FIELD_16(&prInfoElem->u2Version, &u2Version);
	if (prInfoElem->u2Version != 1) {
		DBGPRINT(RT_DEBUG_TRACE, ("Unsupported RSN IE version: %d\n", u2Version));
		return FALSE;
	}

	cp = (PUCHAR)&prInfoElem->u4GroupKeyCipherSuite;
	u4RemainRsnIeLen = (INT32) prInfoElem->ucLength - 2;
	do {
		/* Parse the Group Key Cipher Suite field. */
		if (u4RemainRsnIeLen == 0)
			break;
		if (u4RemainRsnIeLen < 4) {
			DBGPRINT(RT_DEBUG_TRACE, ("Fail to parse RSN IE in group cipher suite (IE len: %d)\n", prInfoElem->ucLength));
			return FALSE;
		}
		...
		if (u4RemainRsnIeLen < 2) {
			DBGPRINT(RT_DEBUG_TRACE, ("Fail to parse RSN IE in pairwise cipher suite count (IE len: %d)\n", prInfoElem->ucLength));
			return FALSE;
		}
		...
		i = (UINT32) u2PairSuiteCount * 4;
		if (u4RemainRsnIeLen < (INT32) i) {
			DBGPRINT(RT_DEBUG_TRACE, ("Fail to parse RSN IE in pairwise cipher suite list (IE len: %d)\n", prInfoElem->ucLength));
			return FALSE;
		}
		pucPairSuite = cp;
		cp += i;
		u4RemainRsnIeLen -= (INT32) i;

		/* Parse the Authentication and Key Management Cipher Suite Count field. */
		if (u4RemainRsnIeLen == 0)
			break;
		if (u4RemainRsnIeLen < 2) {
			DBGPRINT(RT_DEBUG_TRACE, ("Fail to parse RSN IE in auth & key mgt suite count (IE len: %d)\n", prInfoElem->ucLength));
			return FALSE;
		}
		...
		i = (UINT32) u2AuthSuiteCount * 4;
		if (u4RemainRsnIeLen < (INT32) i) {
			DBGPRINT(RT_DEBUG_TRACE, ("Fail to parse RSN IE in auth & key mgt suite list (IE len: %d)\n", prInfoElem->ucLength));
			return FALSE;
		}
		pucAuthSuite = cp;
		cp += i;
		...
		if (u4RemainRsnIeLen == 0)
			break;
		if (u4RemainRsnIeLen < 2) {
			DBGPRINT(RT_DEBUG_TRACE, ("Fail to parse RSN IE in RSN capabilities (IE len: %d)\n", prInfoElem->ucLength));
			return FALSE;
		}
		...
		*/
		...
		if (u4RemainRsnIeLen < 2) {
			DBGPRINT(RT_DEBUG_TRACE, ("Fail to parse RSN IE in PMKID (IE len: %d)\n", prInfoElem->ucLength));
			return FALSE;
		}
		...
		if (u2DesiredPmkidCnt > MAX_NUM_SUPPORTED_PMKID) {
			...
			DBGPRINT(RT_DEBUG_WARN, ("Support maximum PMKID Cnt = %d with desired PMKID Cnt = %d\n", MAX_NUM_SUPPORTED_PMKID, u2DesiredPmkidCnt));
		} else
		...
		if (u2SupportedPmkidCnt > 0)
			memcpy(prRsnInfo->aucPmkidList, cp, (u2SupportedPmkidCnt * RSN_PMKID_LEN));
		...
		if (u4RemainRsnIeLen < 4) {
			DBGPRINT(RT_DEBUG_TRACE, ("Fail to parse RSN IE in GroupMgmtCipher (IELen: %d)\n", prInfoElem->ucLength));
			/* Continued to connect when PMKID List field is truncated. */
			break;
		}
	...
	DBGPRINT(RT_DEBUG_TRACE, ("RSN: version %d, group mgmt key cipher suite %02x-%02x-%02x-%02x\n",
	...
	DBGPRINT(RT_DEBUG_TRACE, ("RSN: version %d, group key cipher suite %02x-%02x-%02x-%02x\n",
	...
	if (pucPairSuite) {
		...
		prRsnInfo->u4PairwiseKeyCipherSuiteCount = (UINT32) u2PairSuiteCount;

		for (i = 0; i < (UINT32) u2PairSuiteCount; i++) {
			WLAN_GET_FIELD_32(pucPairSuite, &prRsnInfo->au4PairwiseKeyCipherSuite[i]);
			pucPairSuite += 4;

			DBGPRINT(RT_DEBUG_TRACE, ("RSN: pairwise key cipher suite [%d]: %02x-%02x-%02x-%02x\n",
			       (UINT8) i,
		...
		DBGPRINT(RT_DEBUG_TRACE, ("RSN: pairwise key cipher suite: %02x-%02x-%02x-%02x (default)\n",
	...
	if (pucAuthSuite) {
		...
		prRsnInfo->u4AuthKeyMgtSuiteCount = (UINT32) u2AuthSuiteCount;

		for (i = 0; i < (UINT32) u2AuthSuiteCount; i++) {
			WLAN_GET_FIELD_32(pucAuthSuite, &prRsnInfo->au4AuthKeyMgtSuite[i]);
			pucAuthSuite += 4;

			DBGPRINT(RT_DEBUG_TRACE, ("RSN: AKM suite [%d]: %02x-%02x-%02x-%02x\n",
			       (UINT8) i,
		...
		DBGPRINT(RT_DEBUG_TRACE, ("RSN: AKM suite: %02x-%02x-%02x-%02x (default)\n",
	...
	DBGPRINT(RT_DEBUG_TRACE, ("RSN cap: 0x%04x\n", prRsnInfo->u2RsnCap));
...
*/
/*----------------------------------------------------------------------------*/
BOOLEAN RTMPSearchAKMSuite(IN PRTMP_ADAPTER pAd, IN UINT32 u4AkmSuite, OUT PUINT8 pu4Index)
{
	UINT8 i;
	P_RSNA_CONFIG_AUTHENTICATION_SUITES_ENTRY prEntry;

	for (i = 0; i < MAX_NUM_SUPPORTED_AKM_SUITES; i++) {
		prEntry = &pAd->RSNAKMSuitesTable[i];
		if (prEntry->dot11RSNAKMSuite == u4AkmSuite && prEntry->dot11RSNAKMSuiteEnabled) {
			*pu4Index = i;
			return TRUE;
		}
	}
...
==========================================================================
*/
BOOLEAN RTMPPerformPolicySelection(IN PRTMP_ADAPTER pAd, IN ULONG BssIdx, IN UCHAR apidx)
{
	BSS_ENTRY *prBssEntry;
	CIPHER_SUITE *prBssRsnInfo;
	UINT8 i, j = 0;
	NDIS_802_11_AUTHENTICATION_MODE AuthMode = Ndis802_11AuthModeMax;
	PNDIS_802_11_WEP_STATUS pWepStatus = NULL;
	NDIS_802_11_ENCRYPTION_STATUS GroupCipher = Ndis802_11EncryptionDisabled;	/* Multicast cipher suite */
	NDIS_802_11_ENCRYPTION_STATUS PairCipher = Ndis802_11EncryptionDisabled;	/* Unicast cipher suite */
	PUCHAR pMixCipher = NULL;	/* Indicate current Pair & Group use different cipher suites */
	PUINT32 pAKMSuite = NULL;
#ifdef CFG_SUPPORT_802_11W
	UINT32 u4Mfp = IW_AUTH_MFP_DISABLED;
	PUCHAR pfgMgmtProtection = NULL;
#endif

	DBGPRINT(RT_DEBUG_TRACE, ("RTMPPerformPolicySelection==>\n"));

	prBssEntry = &pAd->ScanTab.BssEntry[BssIdx];
	...
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			DBGPRINT(RT_DEBUG_TRACE, ("==> RTMPPerformPolicySelection(ApCli)\n"));
			AuthMode = pAd->ApCfg.ApCliTab[apcliIfidx].wdev.AuthMode;
			PairCipher = pAd->ApCfg.ApCliTab[apcliIfidx].PairCipher;
			GroupCipher = pAd->ApCfg.ApCliTab[apcliIfidx].GroupCipher;
			pWepStatus = &pAd->ApCfg.ApCliTab[apcliIfidx].wdev.WepStatus;
			pMixCipher = &pAd->ApCfg.ApCliTab[apcliIfidx].bMixCipher;
			u4Mfp = pAd->ApCfg.ApCliTab[apcliIfidx].u4Mfp;
			pfgMgmtProtection = &pAd->ApCfg.ApCliTab[apcliIfidx].MlmeAux.fgMgmtProtection;
			pAKMSuite = &pAd->ApCfg.ApCliTab[apcliIfidx].AKMSuite;
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("==> RTMPPerformPolicySelection(STA)\n"));

			AuthMode = pAd->StaCfg.wdev.AuthMode;
			PairCipher = pAd->StaCfg.PairCipher;
			GroupCipher = pAd->StaCfg.GroupCipher;
			pWepStatus = &pAd->StaCfg.wdev.WepStatus;
			pMixCipher = &pAd->StaCfg.bMixCipher;
			u4Mfp = pAd->StaCfg.u4Mfp;
			pfgMgmtProtection = &pAd->MlmeAux.fgMgmtProtection;
			pAKMSuite = &pAd->StaCfg.AKMSuite;
	...
	if ((prBssEntry->CapabilityInfo & BIT(4)) == 0) {

		if ((pWepStatus != NULL) && ((*pWepStatus) == Ndis802_11EncryptionDisabled)) {
			DBGPRINT(RT_DEBUG_TRACE, ("-- No Protected BSS\n"));
			return TRUE;
		}
		DBGPRINT(RT_DEBUG_TRACE, ("-- Protected BSS but No need\n"));
		return FALSE;
	}

	/* Protection is required in this BSS. */
	if ((prBssEntry->CapabilityInfo & BIT(4)) != 0) {
		if ((*pWepStatus) == Ndis802_11EncryptionDisabled) {
			DBGPRINT(RT_DEBUG_TRACE, ("-- Protected BSS\n"));
			return FALSE;
		}
	}

	/* Check Whether BSS RSN is matched from upper layer set. */
	if (AuthMode == Ndis802_11AuthModeWPA ||
		AuthMode == Ndis802_11AuthModeWPAPSK ||
		AuthMode == Ndis802_11AuthModeWPANone) {
		prBssRsnInfo = &prBssEntry->WPA;
		...
		prBssRsnInfo = &prBssEntry->WPA2;
	} else if ((*pWepStatus) != Ndis802_11Encryption1Enabled) {
		/* If the driver is configured to use WEP only, ignore this BSS. */
		DBGPRINT(RT_DEBUG_TRACE, ("-- Not WEP-only legacy BSS\n"));
		return FALSE;
	} else if ((*pWepStatus) == Ndis802_11Encryption1Enabled) {
		/* If the driver is configured to use WEP only, use this BSS. */
		DBGPRINT(RT_DEBUG_TRACE, ("-- WEP-only legacy BSS\n"));
		return TRUE;
	}
	/* PairCipher & GroupCipher*/
	DBGPRINT(RT_DEBUG_TRACE, ("Peer u4PairwiseCipherCount %d\n", prBssRsnInfo->u4PairwiseCipherCount));
	if (prBssRsnInfo->u4PairwiseCipherCount > 0) {
		j = 0;
		for (i = 0; i < prBssRsnInfo->u4PairwiseCipherCount; i++) {
			DBGPRINT(RT_DEBUG_TRACE, ("Peer u4PairwiseCipher[%d] %d\n", i, prBssRsnInfo->PairCipher[i]));
			if (PairCipher == prBssRsnInfo->PairCipher[i])
				break;
		}
		if (i == (prBssRsnInfo->u4PairwiseCipherCount)) {
			DBGPRINT(RT_DEBUG_WARN, ("No PairCipher matched\n"));
			return FALSE;
		} else
			DBGPRINT(RT_DEBUG_TRACE, ("Selected PairCipher %d\n", PairCipher));
	}

	if (GroupCipher != prBssRsnInfo->GroupCipher) {
		DBGPRINT(RT_DEBUG_TRACE, ("GroupCipher mismatch :Own %d Peer %d\n", GroupCipher, prBssRsnInfo->GroupCipher));
		return FALSE;
	}
	*pWepStatus = PairCipher;
	DBGPRINT(RT_DEBUG_TRACE, ("Selected GroupCipher %d\n", GroupCipher));
	DBGPRINT(RT_DEBUG_TRACE, ("Selected WepStatus %d\n", (*pWepStatus)));
	*pMixCipher = (GroupCipher == PairCipher) ? FALSE:TRUE;

	/* AKM Suite */
	if (prBssRsnInfo->u4AuthKeyMgtSuiteCount > 0) {
		j = 0;
		for (i = 0; i < prBssRsnInfo->u4AuthKeyMgtSuiteCount; i++) {
			if (RTMPSearchAKMSuite(pAd, prBssRsnInfo->au4AuthKeyMgtSuite[i], &j)) {
				*pAKMSuite = prBssRsnInfo->au4AuthKeyMgtSuite[i];
				break;
			}
		}
		if (i == prBssRsnInfo->u4AuthKeyMgtSuiteCount) {
			DBGPRINT(RT_DEBUG_WARN, ("No AKM Suite matched\n"));
			return FALSE;
		} else
			DBGPRINT(RT_DEBUG_TRACE, ("Selected AKM Suite %x\n", (*pAKMSuite)));
	}

#ifdef CFG_SUPPORT_802_11W
	DBGPRINT(RT_DEBUG_TRACE, ("[MFP] MFP setting = %d\n ", u4Mfp));

	if (u4Mfp == IW_AUTH_MFP_REQUIRED) {
		if (!(prBssRsnInfo->RsnCapability & ELEM_WPA_CAP_MFPC)) {
			DBGPRINT(RT_DEBUG_TRACE, ("[MFP] Skip RSN IE, No MFP Required\n"));
			return FALSE;
		}
		*pfgMgmtProtection = TRUE;
	} else if (u4Mfp == IW_AUTH_MFP_OPTIONAL) {
		if (prBssRsnInfo->RsnCapability & (ELEM_WPA_CAP_MFPR | ELEM_WPA_CAP_MFPC))
			*pfgMgmtProtection = TRUE;
	} else {
		if ((prBssRsnInfo->RsnCapability & ELEM_WPA_CAP_MFPR)) {
			DBGPRINT(RT_DEBUG_TRACE, ("[MFP] Try to join even MFP Required bit set\n"));
			return FALSE;
		}
		*pfgMgmtProtection = FALSE;
	}
	DBGPRINT(RT_DEBUG_TRACE, ("[MFP] fgMgmtProtection = %d\n ", (*pfgMgmtProtection)));
#endif
	return TRUE;
}
	...
	IN BOOLEAN eth_frame)
{
	...
	if (RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU)) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s:Fake-EAPOL as AMSDU flag is on\n", __func__));
		return FALSE;
	}
