...
#define IS_CCMP_CIPHER(pRxBlk) \
	(((pRxBlk)->sec_mode == RXD_SEC_CCMP_MIC) ||\
	((pRxBlk)->sec_mode == RXD_SEC_CCMP_CCX))

#define IS_TKIP_CIPHER(pRxBlk) \
	(((pRxBlk)->sec_mode == RXD_SEC_TKIP_MIC) ||\
	((pRxBlk)->sec_mode == RXD_SEC_TKIP))
...
#define SQ_ENQ_PS_MAX 		768
#define SQ_ENQ_NORMAL_MAX 	768
...
typedef struct _RSN_INFO_ELEM_T {
	...
	UCHAR ucLength;
	UINT16 u2Version;
	UINT32 u4GroupKeyCipherSuite;
	UINT16 u2PairwiseKeyCipherSuiteCount;
	UCHAR aucPairwiseKeyCipherSuite1ist[4];
...
typedef struct _RSN_INFO_T {
	...
	UINT16 u2Version;
	UINT32 u4GroupKeyCipherSuite;
	UINT32 u4PairwiseKeyCipherSuiteCount;
	UINT32 au4PairwiseKeyCipherSuite[MAX_NUM_SUPPORTED_CIPHER_SUITES];
	...
	UINT32 u4GroupMgmtKeyCipherSuite;
...
#ifdef WPA3_SUPPORT
	BOOLEAN fgIsConnInitialized;
	UINT8 ucTxAssocRetryCount;
	UINT8 ucTxAssocRetryLimit;

	/* RSN info store from upper layer */
	RSN_INFO_T RsnInfo;
#ifdef CFG_SUPPORT_802_11W
	UINT32 u4CipherGroupMgmt;
	UINT32 u4Mfp;
...
#endif /* RT_CFG80211_SUPPORT */
	BOOLEAN		bUpdateSeqFromWoWResume;    /* Update the LastIndSeq from WOW resume */
...
#endif /* MAT_SUPPORT */
#ifdef WPA3_SUPPORT
	RSNA_CONFIG_AUTHENTICATION_SUITES_ENTRY RSNAKMSuitesTable[MAX_NUM_SUPPORTED_AKM_SUITES];
#endif
...
typedef struct _RX_BLK
{
	...
	UINT32 sec_mode;		/* 	sec_mode(SEC_mode): Security engine operation mode
							4'h0: None
							4'h1: WEP-40
							4'h2: TKIP w/ MIC
							4'h3: TKIP w/o MIC
