...
#ifdef WPA3_SUPPORT
typedef struct _WLAN_ASSOC_RSP_FRAME {
	/* Association Response MAC header */
	UINT16 u2FrameCtrl;	/* Frame Control */
	UINT16 u2DurationID;	/* Duration */
	...
	UINT16 u2CapInfo;	/* Capability information */
	UINT16 u2StatusCode;	/* Status code */
	UINT16 u2AssocId;	/* Association ID */
	UINT8 aucInfoElem[1];	/* Information elements, such as supported rates, and etc. */
...
typedef struct _WLAN_DEAUTH_FRAME_T {
	...
	UINT16 u2ReasonCode;	/* Reason code */
	UINT8 aucInfoElem[1];	/* Various IEs, possible no. */
...
typedef struct _CIPHER_SUITE {
	...
	USHORT u4PairwiseCipherCount;
	NDIS_802_11_ENCRYPTION_STATUS PairCipher[MAX_NUM_SUPPORTED_CIPHER_SUITES];		/* Unicast cipher 1, this one has more secured cipher suite */
	UINT32 u4AuthKeyMgtSuiteCount;
	UINT32 au4AuthKeyMgtSuite[MAX_NUM_SUPPORTED_AKM_SUITES];
	USHORT RsnCapability;	/* RSN capability from beacon */
	USHORT u2PmkidCnt;
	BOOLEAN aucPmkidList[MAX_NUM_SUPPORTED_PMKID * RSN_PMKID_LEN];
	UINT u4GroupMgmtKeyCipherSuite;
...
#ifdef WPA3_SUPPORT
	UINT u4AuthModeCount;	/* equal to AKM Count :AuthKey Mngmnt Count */
	NDIS_802_11_AUTHENTICATION_MODE AuthMode[MAX_NUM_SUPPORTED_AKM_SUITES];
#else
...
#ifdef WPA3_SUPPORT
/* Entry in SMT dot11RSNAKMSuitesTalbe Table: dot11RSNAKMSuitesEntry */
typedef struct _RSNA_CONFIG_AUTHENTICATION_SUITES_ENTRY {
	UINT32 dot11RSNAKMSuite;
	BOOLEAN dot11RSNAKMSuiteEnabled;
} RSNA_CONFIG_AUTHENTICATION_SUITES_ENTRY, *P_RSNA_CONFIG_AUTHENTICATION_SUITES_ENTRY;
#endif
...
#ifdef WPA3_SUPPORT
	BOOLEAN fgPeerAuthCommitReceived;
	BOOLEAN fgPeerAuthConfirmReceived;
	BOOLEAN fgOwnAuthCommitSend;
	BOOLEAN fgOwnAuthConfirmSend;
#ifdef CFG_SUPPORT_802_11W
	BOOLEAN fgMgmtProtection;
	BOOLEAN fgAPApplyPmfReq;
	UINT32 u4SaQueryStart;
	UINT32 u4SaQueryCount;
	UINT8 ucSaQueryTimedOut;
	PUINT8 pucSaQueryTransId;
