...
*/
	...
	if (pRxInfo->Bcast || pRxInfo->Mcast) {
		...
		if (pHeader->FC.MoreFrag) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s:drop B/Mcast fragment!\n", __func__));
			RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_FAILURE);
			return;
		}
		
		if (((pHeader->FC.Wep == 1) && (pEntry->WepStatus == Ndis802_11EncryptionDisabled)) ||
			((pHeader->FC.Wep == 0) && (pEntry->WepStatus != Ndis802_11EncryptionDisabled))) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s:drop B/Mcast pkt as FC.Wep(%d) mismatch with pEntry->WepStatus(%d)!\n", __func__, pHeader->FC.Wep, pEntry->WepStatus));
                        RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_FAILURE);
                        return;
		}
		...
		if (pHeader->FC.FrDs
			...
			DBGPRINT(RT_DEBUG_ERROR, ("%s:drop AP relay B/Mcast!\n", __func__));
			RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_FAILURE);
			return;
		}
