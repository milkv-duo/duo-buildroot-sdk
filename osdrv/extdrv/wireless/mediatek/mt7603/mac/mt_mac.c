	...
	IN HTTRANSMIT_SETTING *pTransmit)
{
	...
	/* DWORD 0 */
	/* txd_0->p_idx = (to_mcu ? P_IDX_MCU : P_IDX_LMAC); fix coverity issue: to_mcu must be 0 */
	UNUSED(to_mcu);
	...
	if (txd_1->ft == TMI_FT_LONG) {
		...
		else
		{
			...
			if (TxSStatusPerWlanIdx & (1 << (txd_1->wlan_idx & 0x1f)))
			{
...
VOID write_tmac_info_Data(RTMP_ADAPTER *pAd, UCHAR *buf, TX_BLK *pTxBlk)
{
	...
	/* DWORD 0 */
	UNUSED(to_mcu);
	/* txd_0->p_idx = (to_mcu ? P_IDX_MCU : P_IDX_LMAC); to fix coverity issue, to_mcu == 0 */
	txd_0->p_idx = P_IDX_LMAC;
	if (pTxBlk->QueIdx < 4) {
...
//CFG TODO
	...
	if (txd_long == FALSE)
	{
		...
		else
		{
			...
			if (TxSStatusPerWlanIdx & (1 << (txd_1->wlan_idx & 0x1f))) /* shifting bits should not more than 31 */
			{
				if (TxSCtl->TxSFormatPerPktType[txd_2->frm_type] & (1 << txd_2->sub_type))
					txd_5->tx_status_fmt = TXS_FORMAT1;
				else
