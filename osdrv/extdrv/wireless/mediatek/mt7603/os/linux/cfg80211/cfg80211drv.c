...
#endif /*CONFIG_STA_SUPPORT*/
#ifdef WPA3_SUPPORT
		case CMD_RTPRIV_IOCTL_80211_P2PCLI_AUTH_IE_SET:
			CFG80211DRV_SetP2pCliAuthIe(pAd, pData, Data);
			break;
#endif
	...
	VOID						*pData)
{
...
#ifdef WPA3_SUPPORT
	memset(&Bssid, 0, sizeof(NDIS_802_11_MAC_ADDRESS));
	memcpy(Bssid, pConnInfo->pBssid, sizeof(NDIS_802_11_MAC_ADDRESS));
...
#ifdef WPA3_SUPPORT
		Set_BSSID_Proc(pAd, (PUINT8)Bssid, pConnInfo->ucChannelNum);
#else
	...
	/* set channel Num to pAd->StaCfg */
	pAd->MlmeAux.Channel = pConnInfo->ucChannelNum;
#endif
...
#ifdef WPA3_SUPPORT
	Set_BSSID_Proc(pAd, (PUINT8)Bssid, pConnInfo->ucChannelNum);
	CFG80211DBG(RT_DEBUG_TRACE, ("80211> Connecting BSSID [%02x:%02x:%02x:%02x:%02x:%02x]\n", PRINT_MAC(pConnInfo->pBssid)));
#else
	Set_SSID_Proc(pAd, (RTMP_STRING *)SSID);
