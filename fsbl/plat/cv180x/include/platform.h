...
static inline uintptr_t phys_to_dma(uintptr_t phys)
{
	...
	if (IN_RANGE(phys, VC_RAM_BASE, VC_SRAM_SIZE))
		dma = phys - VC_RAM_BASE + VC_SRAM_ORIGIN_BASE;
...
static inline uintptr_t dma_to_phys(uintptr_t dma)
{
	...
	if (IN_RANGE(dma, VC_SRAM_ORIGIN_BASE, VC_SRAM_SIZE))
		phys = dma - VC_RAM_BASE + VC_SRAM_ORIGIN_BASE;
