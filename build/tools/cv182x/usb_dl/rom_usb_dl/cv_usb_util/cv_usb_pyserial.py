...
class cv_usb_pyserial:
    ...
    def serial_query(self, vid_pid_list, timeout=0, verify=0, location=None):
        ...
        while (found == -1):
            time.sleep(0.05)
            i = i + 1
            sys.stdout.write("Waiting for USB port: %s    \r" %
        ...
        while (connect == -1):
            ...
            try:
                ...
                if timeout != 0 and ((time.time() - query_time) > timeout):
                    ...
                    sys.stdout.flush()
                    sys.exit(-1)
                sys.stdout.write("Waiting for USB connect: %s    \r" %
                                 (progress_symbol[i % len(progress_symbol)]))
                sys.stdout.flush()
                time.sleep(0.05)
    ...
    def serial_write(self, command, recv_ack, delay_ms):
        ...
        if recv_ack == 1:
            ...
            if ret_crc == cmd_crc:
                ...
                print("ACK_CRC_ERROR cmd_crc %x; ret_crc %x" %
                      (cmd_crc, ret_crc))
                return pkt.FAIL
        else:
            return pkt.SUCCESS

    def usb_send_2nd_fip(self, filename, dest_addr, delay_ms, last_pos=0):
        ...
        while (complete_cnt < 1):  # For stress test
            ...
            while (content_size > 0):
                ...
                if filename == 'cv_dl_magic.bin':
                    ...
                    self.header.append(pkt.CVI_USB_TX_DATA_TO_DRAM)
        ...
        while (complete_cnt < 1):  # For stress test
            ...
            while (content_size > 0):
                ...
                if Type == "magic":
                    ...
                    self.header.append(pkt.CVI_USB_TX_DATA_TO_SRAM)
