...
#define UUID_BLP \
    {0xf6ca4974, 0x6bc9, 0x427c, 0x82, 0x2c, {0xcb, 0x3a, 0x4f, 0x6a, 0x88, 0x7a} }
#define UUID_DDRC \
    {0xd7456ca3, 0x3ac1, 0x4c38, 0x8c, 0x93, {0x19, 0x12, 0xa4, 0x62, 0x45, 0x34} }
...
class FIP:
    ...
    def load(self):
        ...
        if len(rest) != 0:
            logging.warning("rest size is %d", len(rest))

        loc = rest.find(b"APLB")
        self.old_blp_binary = b""
        if loc >= 0:
            logging.warning("Old BLD binary found at %a", loc)
            self.old_blp_binary = rest[loc:]

        self.ents = collections.OrderedDict(ents)

        try:
            if not self.ents["BLP"].content.startswith(b"APLB"):
                logging.error("Invalid BLP.")
        except KeyError:
            logging.warning("BLP not found")

        try:
            if not self.ents["DDRC"].content.startswith(b"PDLB"):
                logging.error("Invalid DDRC.")
        except KeyError:
            logging.warning("DDRC not found")
...
def tar_bld(fip_path, output_path):
    logging.debug("tar_bld: %s", fip_path)
    fip = FIP(fip_path)
    fip.load()

    members = ["BLD_CONTENT_CERT", "BLD2_KEY_CERT", "BLD1_KEY_CERT", "BLD"]
...
def read_bld_tar(inputs, bld_tar_path):
    logging.info("Open %s", bld_tar_path)
    members = [
...
def main():
    ...
    if args.tar_bld:
        tar_bld(args.FIP_BIN[0], args.output)

    inputs = collections.OrderedDict()
    for name in list(FIP.ENTRY_NAMES):
    ...
    if args.BLD_TAR:
        read_bld_tar(inputs, args.BLD_TAR)
