    ...
    def send_chunk(self, chunk, size, dest_addr, delay_ms, Type="file"):
        if self.driver == "pyserial":
            ...
            if Type == "file":
                ...
                self.usb_dev.serial_send_chunk(chunk, size, dest_addr,
                                               delay_ms, Type)

    def send_req_data(self, token, address, reqLen, data=None):
        if self.driver == "pyserial":
            return self.usb_dev.usb_send_req_data(token, address, reqLen, data)
        else:
            return self.usb_dev.usb_send_req_data(token, address, reqLen, 0, data)

    def send_req_data_rsp(self, token, address, reqLen, data=None):
        if self.driver == "pyserial":
            return self.usb_dev.usb_send_req_data_rsp(token, address, reqLen, data)
        else:
            return self.usb_dev.usb_send_req_data_rsp(token, address, reqLen, 0, data)
