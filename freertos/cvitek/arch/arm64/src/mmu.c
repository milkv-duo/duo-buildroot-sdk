...
static struct mm_region vexpress64_mem_map[] = {
	...
	{ .virt = 0x120000000UL,
	  .phys = 0x120000000UL,
	  .size = 0x001000000UL,
	  .attrs = PTE_BLOCK_MEMTYPE(MT_NORMAL_NC) | PTE_BLOCK_NON_SHARE |
...
/* Move the ___memset from main.c for mmu.c use only. */
...
static uint64_t *create_table(void)
{
	...
	if (g_tlb_fillptr - g_tlb_addr > g_tlb_size)
		panic("Insufficient RAM for page table: 0x%lx > 0x%lx. "
		      ...
		      g_tlb_fillptr - g_tlb_addr, g_tlb_size);

	/* Mark all entries as invalid */
	____memset(new_table, 0, pt_len);
...
/* MMU_table need to alignment 4KB and this would not be changed now, so we use array here to reduce code size*/
char mmu_table_entry[(MAX_PTE_ENTRIES * sizeof(uint64_t)) * PAGE_TABLE_NUM]
