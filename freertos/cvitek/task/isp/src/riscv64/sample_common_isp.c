...
static CVI_VOID *SAMPLE_COMM_ISP_Thread(void *arg)
{
	...
	// CVI_MISC_RegisterThermalCallback(callback_FPS);
...
out:
	...
	printf("%s\n", __func__);
	CVI_S32 s32Ret = 0;
	CVI_U8 *arg = pvPortMalloc(sizeof(*arg));
...
CVI_VOID *SAMPLE_COMM_ISP_GetSnsObj(uint32_t u32SnsId)
{
	SAMPLE_SNS_TYPE_E enSnsType;

	enSnsType = g_enSnsType[u32SnsId];
	return CVI_GetSnsObj(enSnsType);
}
	...
	for (i = 0; i < pstViConfig->s32WorkingViNum; i++) {
		...
		if (s32Ret != CVI_SUCCESS) {
			CVI_TRACE_LOG(CVI_DBG_ERR, "sensor set image mode failed! \
				stSnsrMode.u16Width %d stSnsrMode.u16Height %d %f wdrMode %d pstSnsObj %p\n",
...
static SNS_BDG_MUX_MODE_E SAMPLE_COMM_ISP_GetSnsBdgMode(CVI_U32 u32SnsId, SAMPLE_SNS_TYPE_E enSnsType)
{
	VI_DEV_ATTR_S       stViDevAttr;
	SNS_BDG_MUX_MODE_E  MuxMode;

	SAMPLE_COMM_VI_GetDevAttrBySns(u32SnsId, enSnsType, &stViDevAttr);
						...
						CVI_S32 s32I2cAddr)
{
	...
	pstInitAttr->enGainMode = SNS_GAIN_MODE_SHARE;//SNS_GAIN_MODE_WDR_2F
	...
	pstInitAttr->enSnsBdgMuxMode = SAMPLE_COMM_ISP_GetSnsBdgMode(u32SnsId, enSnsType);
