...
#if 0
	CVI_BOOL bUsrMode;
	/* mode 0: auto mode 1: mannual.*/
	CVI_S8 s8CngMode;
	/* cozy noisy mode: 0 close, 1 open, recommend 1*/
	CVI_S8 s8NearAllPassEnergy;
	/* the far-end energy threshold for judging whether */
	/*unvarnished transmission: 0 -59dBm0, */
	/*1 -49dBm0, 2 -39dBm0, recommend 1 */
	CVI_S8 s8NearCleanSupEnergy;
	/* the energy threshold for compelling reset */
	/*of near-end signal: 0 12dB, 1 15dB, 2 18dB, recommend 2 */
	CVI_S16 s16DTHnlSortQTh;
	/* the threshold of judging single or */
	/*double talk, recommend 16384, [0, 32767] */
	CVI_S16 s16EchoBandLow;
	/* voice processing band1, low frequency */
	/*parameter, [1, 63) for 8k, [1, 127) for 16k, recommend 10 */
	CVI_S16 s16EchoBandHigh;
	/* voice processing band1, high frequency parameter, */
	/*(s16EchoBandLow, 63] for 8k, */
	/*(s16EchoBandLow, 127] for 16k, recommend 41 */
	/* s16EchoBandHigh must be greater than s16EchoBandLow */
	CVI_S16 s16EchoBandLow2;
	/* voice processing band2, low frequency parameter,  */
	/* [1, 63) for 8k, [1, 127) for 16k, recommend 47 */
	CVI_S16 s16EchoBandHigh2;
	/* voice processing band2, high frequency parameter, */
	/*(s16EchoBandLow2, 63] for 8k, */
	/*(s16EchoBandLow2, 127] for 16k, recommend 72 */
	/* s16EchoBandHigh2 must be greater than s16EchoBandLow2 */
	CVI_S16 s16ERLBand[6];
	/* ERL protect area, [1, 63] for 8k, [1, 127] for 16k,  */
	/*frequency band calculated by s16ERLBand * 62.5 */
	/* besides, s16ERLBand[n+1] should be greater than s16ERLBand[n] */
	CVI_S16 s16ERL[7];
	/* ERL protect value of ERL protect area, the */
	/*smaller its value, the more strength its protect ability,[0, 18]*/
	CVI_S16 s16VioceProtectFreqL;
	/* protect area of near-end low frequency, */
	/*[1, 63) for 8k, [1, 127) for 16k, recommend 3 */
	CVI_S16 s16VioceProtectFreqL1;
	/* protect area of near-end low frequency1, */
	/*(s16VioceProtectFreqL, 63] for 8k, (s16VioceProtectFreqL, 127]*/
	/*for 16k, recommend 6 */
#else
	CVI_CHAR *cfg_data;
	unsigned short para_aec_filter_len;
	//unsigned int para_aec_supp16k_ena;
	unsigned short para_aes_std_thrd;                 /* the threshold of STD/DTD, [0, 35] */
	unsigned short para_aes_supp_coeff;           /* the residual echo suppression level in AES, [0, 24] */

	CVI_S32 s32Reserved;
	/* s16VioceProtectFreqL1 must be greater */
	/*than s16VioceProtectFreqL */
#endif
} AI_AEC_CONFIG_S;
...
#if 0
	CVI_BOOL bUsrMode;   /* mode 0: auto, mode 1: manual.*/
	CVI_S16 s16NrIntensity;
	/* noise reduce intensity, range: [0, 25] */
	CVI_S16 s16NoiseDbThr;
	/* noise threshold, range: [30, 60] */
	CVI_S8  s8SpProSwitch;
	/* switch for music probe, range: [0:close, 1:open] */
#else
...
typedef struct _AUDIO_AGC_CONFIG_S {
	...
	CVI_S8 s8TargetLevel;
	/* target voltage level, range: [-40, -1]dB */
	CVI_S8 s8NoiseFloor;
	/* noise floor, range: TalkVqe/AoVqe[-65, -20]dB,*/
	/*RecordVqe[-50, -20]dB */
	CVI_S8 s8MaxGain;
	/* max gain, range: [0, 30]dB */
	CVI_S8 s8AdjustSpeed;
	/* adjustable speed, range: [0, 10]dB/s */

	CVI_S8 s8ImproveSNR;
	/* switch for improving SNR, range: */
	/*[0:close, 1:upper limit 3dB, 2:upper limit 6dB] */
	CVI_S8 s8UseHighPassFilt;
	/* switch for using high pass filt, */
	/*range: [0:close, 1:80Hz, 2:120Hz, */
	/*3:150:Hz, 4:300Hz: 5:500Hz] */
	CVI_S8 s8OutputMode;
	/* output mode, mute when lower than */
	/*noise floor, range: [0:close, 1:open] */
	CVI_S16 s16NoiseSupSwitch;
	/* switch for noise suppression, */
	/*range: [0:close, 1:open] */
#else /* CVI_ALGO agc parameter ver20200110 */
	...
	CVI_BOOL para_agc_vad_enable;
	/* the time to keep AGC VAD rising after speech VAD drops */
	/* need to collocate with para_agc_vad_enable, [1, 25] */
	/* para_obj.para_agc_vad_cnt = 13; */
	CVI_S8 para_agc_vad_cnt;
	/* to allow cutting 6dB automatically on max gain to control */
	/* not to amplify too much, [0, 1] */
	/* para_obj.para_agc_cut6_enable = 1; */
	CVI_BOOL para_agc_cut6_enable;
#endif
...
typedef struct _AO_VQE_CONFIG_S {
	CVI_U32	 u32OpenMask;
	CVI_S32 s32WorkSampleRate;
	/* Sample Rate: 8KHz/16KHz/48KHz. default: 8KHz*/
	CVI_S32              s32FrameSample;
	/* VQE frame length: 80-4096 */
	VQE_WORKSTATE_E     enWorkstate;
	AUDIO_ANR_CONFIG_S stAnrCfg;
