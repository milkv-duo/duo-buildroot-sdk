...
typedef enum _RC_SUPERFRM_MODE_E {
...
typedef struct _VENC_PARAM_H264_CBR_S {
	...
	CVI_U32 u32MaxQp; /* RW; Range:(MinQp, 51];the max QP value */
	CVI_U32 u32MinQp; /* RW; Range:[0, 51]; the min QP value */
	CVI_U32 u32MaxIQp; /* RW; Range:(MinQp, 51]; max qp for i frame */
...
typedef struct _VENC_PARAM_H264_VBR_S {
	...
	CVI_U32 u32MaxQp; /* RW; Range:(MinQp, 51]; the max P B qp */
	CVI_U32 u32MinQp; /* RW; Range:[0, 51]; the min P B qp */
	CVI_U32 u32MaxIQp; /* RW; Range:(MinIQp, 51]; the max I qp */
...
typedef struct _VENC_PARAM_H264_AVBR_S {
	...
	CVI_U32 u32MaxStillQP; /* RW; Range:[MinIQp, MaxIQp]; the max QP value of I frame for still scene*/
	...
	// Difference between FrameLevelMinQp & MinQp,FrameLevelMinQp = MinQp(or MinIQp) + MinQpDelta
...
typedef struct _VENC_PARAM_H265_CBR_S {
	CVI_U32 u32MinIprop; /* RW; Range: [u32MinIprop, 100]; the min ratio of i frame and p frame */
	CVI_U32 u32MaxIprop; /* RW; Range: [1, 100];the max ratio of i frame and p frame */
	CVI_U32 u32MaxQp; /* RW; Range:(MinQp, 51];the max QP value */
	CVI_U32 u32MinQp; /* RW; Range:[1, 51];the min QP value */
	CVI_U32 u32MaxIQp; /* RW; Range:(MinIQp, 51];max qp for i frame */
	CVI_U32 u32MinIQp; /* RW; Range:[1, 51];min qp for i frame */
	CVI_S32 s32MaxReEncodeTimes; /* RW; Range:[0, 3]; Range:max number of re-encode times.*/
	CVI_BOOL bQpMapEn; /* RW; Range:[0, 1]; enable qpmap.*/
...
typedef struct _VENC_PARAM_H265_VBR_S {
	...
	CVI_U32 u32MaxIprop; /* RW; (MinIprop, 100]the max ratio of i frame and p frame */
	CVI_S32 s32MaxReEncodeTimes; /* RW; Range:[0, 3]; Range:max number of re-encode times.*/
...
typedef struct _VENC_PARAM_H265_AVBR_S {
	...
	CVI_S32 s32MinStillPercent; /* RW; Range:[20, 100]; the min percent of target bitrate for still scene */
	CVI_U32 u32MaxStillQP; /* RW; Range:[MinIQp, MaxIQp]; the max QP value of I frame for still scene*/
	CVI_U32 u32MinStillPSNR; /* RW; reserved */

	CVI_U32 u32MaxQp; /* RW; Range:(MinQp, 51];the max P B qp */
	CVI_U32 u32MinQp; /* RW; Range:[1, 51];the min P B qp */
	CVI_U32 u32MaxIQp; /* RW; Range:(MinIQp, 51];the max I qp */
	CVI_U32 u32MinIQp; /* RW; Range:[1, 51];the min I qp */
	CVI_U32 u32MinQpDelta;
	// Difference between FrameLevelMinQp & MinQp, FrameLevelMinQp = MinQp(or MinIQp) + MinQpDelta
	CVI_U32 u32MotionSensitivity; /* RW; Range:[0, 100]; Motion Sensitivity */
...
typedef struct _VENC_SUPERFRAME_CFG_S {
	VENC_SUPERFRM_MODE_E
	enSuperFrmMode; /* RW; Indicates the mode of processing the super frame */
	CVI_U32 u32SuperIFrmBitsThr; // RW; Range:[0, 4294967295];Indicate the threshold
	// of the super I frame for enabling the super frame processing mode
	CVI_U32 u32SuperPFrmBitsThr; // RW; Range:[0, 4294967295];Indicate the threshold
	// of the super P frame for enabling the super frame processing mode
	CVI_U32 u32SuperBFrmBitsThr; // RW; Range:[0, 4294967295];Indicate the threshold
